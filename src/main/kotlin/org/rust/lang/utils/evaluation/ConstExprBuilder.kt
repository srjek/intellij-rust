...
fun RsExpr.toConstExpr(
    ...
    val builder = when (expectedTy) {
        is TyInteger -> IntegerConstExprBuilder(expectedTy, pathExprResolver)
        is TyBool -> BoolConstExprBuilder(pathExprResolver)
        is TyFloat -> FloatConstExprBuilder(expectedTy, pathExprResolver)
        is TyChar -> CharConstExprBuilder(pathExprResolver)
        // TODO: type should be "wider"
        STR_REF_TYPE -> StrConstExprBuilder(pathExprResolver)
        else -> null
    }
...
private abstract class ConstExprBuilder<T : Ty, V> {
    ...
    protected abstract val pathExprResolver: ((RsPathExpr) -> RsElement?)?
...
private class IntegerConstExprBuilder(
    ...
    override val pathExprResolver: ((RsPathExpr) -> RsElement?)?
